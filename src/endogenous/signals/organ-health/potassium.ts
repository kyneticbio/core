...
export const potassium: SignalDefinition = {
  ...
  idealTendency: "mid",
  dynamics: {
    setpoint: (ctx, state) => ctx.subject?.bloodwork?.metabolic?.potassium_mmol_L ?? 4.2,
    tau: 10080,
    production: [],
    clearance: [],
    couplings: [],
  },
  initialValue: (ctx) => ctx.subject?.bloodwork?.metabolic?.potassium_mmol_L ?? 4.2,
  display: {
  ...
  monitors: [
    {
      ...
      description: "Low potassium can cause muscle weakness and cardiac arrhythmias.",
    },
    {
      id: "high_potassium",
      signal: "potassium",
      ...
      description: "Elevated potassium can be dangerous and affect heart rhythm.",
    },
  ],
};
