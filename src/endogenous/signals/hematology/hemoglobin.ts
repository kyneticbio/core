...
export const hemoglobin: SignalDefinition = {
  ...
  idealTendency: "mid",
  dynamics: {
    setpoint: (ctx, state) => ctx.subject?.bloodwork?.hematology?.hemoglobin_g_dL ?? 14.5,
    tau: 10080,
    production: [],
    clearance: [],
    couplings: [],
  },
  initialValue: (ctx) => ctx.subject?.bloodwork?.hematology?.hemoglobin_g_dL ?? 14.5,
  display: {
  ...
  monitors: [
    {
      ...
      description: "Low hemoglobin reduces oxygen delivery to tissues, causing fatigue and weakness.",
    },
  ],
};
